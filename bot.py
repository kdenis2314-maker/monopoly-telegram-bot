...
def main():
    ...
    try:
        ...
        add_web_log(f"Токен бота: {'✅ Установлен' if TOKEN else '❌ Отсутствует'}", "INFO")
        add_web_log(f"Домен: {RENDER_DOMAIN}", "INFO")
        add_web_log(f"Вебхук URL: {WEBHOOK_URL}", "INFO")
        add_web_log(f"Порт: {PORT}", "INFO")
        add_web_log("=" * 50, "INFO")
        ...
        if bot is None:
            add_web_log("❌ Бот не был инициализирован. Проверьте BOT_TOKEN.", "ERROR")
            print("❌ Бот не был инициализирован. Проверьте переменную окружения BOT_TOKEN.")
        else:
            add_web_log("✅ Бот успешно инициализирован и готов к работе!", "INFO")
            print("✅ Бот успешно инициализирован!")
        
        # Запуск Flask приложения
        add_web_log(f"🌐 Запуск Flask на порту {PORT}", "INFO")
